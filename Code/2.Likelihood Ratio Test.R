...
# 假设你有无 ARMA 和有 ARMA 的对数似然值
# logLik_values_0 是无 ARMA 动态模型的对数似然值
# logLik_values_1 是有 ARMA 动态模型的对数似然值

for (i in 1:10) {
  # 计算似然比检验统计量 LRT
  LRT_values[i] <- -2 * (logLik_values_0[i] - logLik_values_1[i])
  
  # 计算 p 值，假设自由度为 2（因为 ARMA(1,1) 增加了两个参数）
